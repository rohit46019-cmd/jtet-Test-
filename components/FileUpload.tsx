...
const FileUpload: React.FC<FileUploadProps> = ({ onFileSelect, isLoading }) => {
  ...
  return (
    <div className="w-full max-w-2xl mx-auto">
      <div 
        className={`relative flex flex-col items-center justify-center w-full h-64 border-2 border-dashed rounded-[2.5rem] transition-all duration-300 ${
          dragActive ? "border-blue-500 bg-blue-50 shadow-inner" : "border-slate-300 bg-white dark:bg-slate-900"
        } ${isLoading ? "opacity-50 cursor-not-allowed" : "cursor-pointer hover:border-blue-400 hover:bg-slate-50 dark:hover:bg-slate-800"}`}
